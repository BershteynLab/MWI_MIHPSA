...
write_excel_csv(baseline.comp.data, "/gpfs/data/bershteynlab/EMOD/citrod01/MWI_calib_20220422/transmission_baseline_comp_data.csv")
```
...
# Dry run through Transmission reports data
...
Because it is so much trickier when we need to keep track of the full timelines of people's lives, will need to keep these three tables separated for now, and perform a series of clever joins to subset the data. Anything less than this will be too complicated to assimilate all this together.

```{r Undiagnosed}
# Need to know all the people who transmitted before thefy got diagnosed
...
# Subtract
# This isn't working because we have negatives. What am I doing wrong? Need to take the first time somoene inits art only
...
fun.onart.vl <- function(tr.data, art.report, year.age.sex){
  ...
  dat.onart.vl <- p1 %>% 
    mutate(OnART.VL.Supp = .92 * OnART, OnART.VL.NoSupp = .08 * OnART) %>% 
...
for (i in range(1:3)){ #range(1:length(folder.list))){
...
```{r}
dat.transm.output <- read_csv("/gpfs/data/bershteynlab/EMOD/citrod01/MWI_calib_20220422/Analysis/mihpsa_transmissions_table.csv")
```

```{r}
# These are the columns that loveleen wanted
dat.transm.output %>% 
  select(Tr_year_binned, Age_bin, Undiagnosed.total, Diagnosed_noART.total, OnART.VL.Supp.total, OnART.VL.NoSupp.total, DropOut.total) %>%
  filter(Age_bin == 20)
```
...
dat.transm.all.list = list()


for (i in range(1:length(folder.list))){
  tr.data = tr.data.list[[i]]
  
  p1 <- tr.data %>% filter(Tr_year_binned >= 2000) %>% 
    group_by(Tr_year_binned, Age_bin, Gender) %>% count() %>% rename("n_all" = n)
  ...
  dat.transm <- p %>% mutate(gcode = case_when(Gender == 0 ~ 'm', Gender == 1 ~ 'f')) %>% 
    pivot_wider(id_cols = c(Tr_year_binned, Age_bin),
                names_from = c(gcode), 
                values_from = c(n_all), 
                names_prefix = 'All.', names_sep = '') %>%
    mutate(All.total = All.m + All.f) %>% 
    arrange(Age_bin, Tr_year_binned) %>% select(c(1,2,5,3,4))
  
  
  dat.transm$sim.id <- i
  
  dat.transm.all.list[[i]] <- dat.transm
}

dat.transm.all.list <- bind_rows(dat.transm.all.list)
  
dat.transm.all.list <- dat.transm.all.list %>% 
  inner_join(
    sim.results.pop.scaling %>% select(pop.scaling.factor, sim.id = sim.id.ix),
    by = c("sim.id")
  )
...
dat.transm.all.list %>% 
  group_by(Tr_year_binned, Age_bin) %>% 
  summarize(All.total = mean(All.total*pop.scaling.factor)) %>% 
  arrange(Age_bin, Tr_year_binned) %>% 
  filter(Age_bin == 15)
...
```{r}
dat.transm.all.list %>% select(Tr_year_binned, Age_bin, All.total, sim.id) %>% 
  filter(Age_bin == 20) %>% 
  inner_join(
    sim.results.pop.scaling %>% select(pop.scaling.factor, sim.id = sim.id.ix),
    by = c("sim.id")
  ...
  group_by(Tr_year_binned, Age_bin) %>% 
  summarize(All.total = mean(All.total * pop.scaling.factor), .groups = 'keep')
```


```{r}
dat.transm.list %>% 
  select(Tr_year_binned, Age_bin, Undiagnosed.total, Diagnosed_noART.total, OnART.VL.Supp.total, OnART.VL.NoSupp.total, DropOut.total, sim.id) %>%
  mutate(All.total =  Undiagnosed.total + Diagnosed_noART.total + OnART.VL.Supp.total + OnART.VL.NoSupp.total + DropOut.total) %>% 
  filter(Age_bin == 20) %>% 
  inner_join(
    sim.results.pop.scaling %>% select(pop.scaling.factor, sim.id = sim.id.ix),
    by = c("sim.id")
...
```{r}
dat.transm.list %>% group_by(Tr_year_binned, Age_bin) %>% 
  summarize(Undiagnosed.total = mean(Undiagnosed.total*pop.scaling.factor))
```
...
dat.transm.list %>% 
  ...
  group_by(Tr_year_binned, Age_bin) %>% 
  summarize(Undiagnosed.total = mean(Undiagnosed.total * pop.scaling.factor), .groups = 'keep')
```

New suggested figure: breakdown of transmission events by age
