...
```{r}
input_path = "/gpfs/data/bershteynlab/EMOD/citrod01/MWI_calib_20220422/MWI_transmission/Baseline-campaign_Malawi_20220422_recalib-transmission_ARTStatus/ReportHIVByAgeAndGender/"

sim.results.artstatus <- EMODAnalyzeR::read.simulation.results(
  results_path = input_path,
  scenario_name = 'baseline',
  summarize_columns = c("Newly.Infected", "Newly.Tested.Positive", "Newly.Tested.Negative",
                        "Population","Infected", "On_ART","Died", "Died_from_HIV",
                        "Tested.Past.Year.or.On_ART", "Tested.Ever","Diagnosed" ),
  stratify_columns = c("Year", "Gender", "Age", "IP_Key.ARTStatus"),
  min_age_inclusive = 0,
  max_age_inclusive = 99
) %>% 
  ungroup()
...
sim.results.pop.scaling <- sim.results.artstatus %>% 
      filter(Year == CENSUS_YEAR) %>%
      group_by(sim.id) %>%
      summarize(total.pop = sum(Population), .groups = 'keep') %>% 
      mutate(pop.scaling.factor = MALAWI_CENSUS_POP/total.pop)
...
sim.results.artstatus <- sim.results.artstatus %>% 
  inner_join(
    sim.results.pop.scaling,
    by = c("sim.id")
  )
...
Check: anyone on art who is naive
```{r}
sim.results.artstatus %>% filter(IP_Key.ARTStatus == "ARTNaive") %>% 
  summarize(sum(On_ART))
```

Check: anyone on art who is not literally on art
```{r}
sim.results.artstatus %>% filter(IP_Key.ARTStatus != "OnART") %>% 
  summarize(sum(On_ART))
```

Check: anyone off art who is literally on art

```{r}
sim.results.artstatus %>% filter(IP_Key.ARTStatus == "OffART") %>% 
  summarize(sum(On_ART))
```
Check: does the number of people counted by OnART column equal the number of people IP=OnART?
```{r}
sim.results.artstatus %>% filter(IP_Key.ARTStatus == "OnART") %>% 
...
```{r}
# dat.undiagnosed <- sim.results.artstatus %>% group_by(Year, Gender, Age, sim.ix) %>% 
...
#### Counting On ART
```{r}
dat.onart <- sim.results.artstatus %>% group_by(Year, Gender, Age, sim.ix) %>% 
...
#### Counting ART Naive Diagnosed individuals

```{r}
dat.artnaive <- sim.results.artstatus %>% filter(IP_Key.ARTStatus == "ARTNaive") %>%
  group_by(Year, Gender, Age, sim.ix) %>% 
  summarize(Diagnosed.ArtNaive = sum(Diagnosed* pop.scaling.factor), .groups = 'keep') %>% 
  group_by(Year, Gender, Age) %>% 
  summarize(Diagnosed.ArtNaive = mean(Diagnosed.ArtNaive), .groups = 'keep')
```

#### Counting Off ART

```{r}
dat.art.dropout <- sim.results.artstatus %>% filter(IP_Key.ARTStatus == "OffART") %>%
  group_by(Year, Gender, Age, sim.ix) %>% 
  summarize(ArtDropout = sum(Population * pop.scaling.factor), .groups = 'keep') %>% 
  group_by(Year, Gender, Age) %>% 
  summarize(ArtDropout = mean(ArtDropout), .groups = 'keep')
...
dat.plhiv <- sim.results.artstatus %>% 
  group_by(Year, Gender, Age, sim.ix) %>% 
...
dat <- merge(
  ...
  merge(dat.onart,
        by = c("Year", "Gender", "Age")) %>% 
  merge(dat.art.dropout,
        by = c("Year", "Gender", "Age")) %>%
  mutate(Undiagnosed = PLHIV - Diagnosed.ArtNaive - On_ART - ArtDropout)
...
```{r}
write_excel_csv(dat, "mihpsa_art_status.csv")
```


#### Checks

Check to see if undiagnosed + art naive + on art + art dropout all add up to total PLHIV
...
dat %>%
  mutate(tot = (Undiagnosed + Diagnosed.ArtNaive + On_ART+ ArtDropout )) %>% 
  mutate(diff = PLHIV - tot) %>% View

```

Check: does the number of Naive, On Art, Off art people equal the number infected?

```{r}
sim.results.artstatus %>% 
  pivot_wider(
    id_cols = c("Year", "Gender", "Age", "sim.ix"),
    names_from = IP_Key.ARTStatus,
    values_from = Infected
  )
```

Check: total numbers infected are the same across both sim versions?

```{r}
dat.plhiv.old <- sim.results %>% 
  group_by(Year, Gender, Age, sim.ix) %>% 
  summarize(PLHIV = sum(Infected * pop.scaling.factor), .groups = 'keep') %>% 
  group_by(Year, Gender, Age) %>% 
  summarize(PLHIV = mean(PLHIV), .groups = 'keep')
...
merge(
  dat.plhiv.old %>% select(Year, Gender, Age, PLHIVold= PLHIV),
  dat.plhiv %>% select(Year, Gender, Age, PLHIVnew= PLHIV),
  by = c("Year", "Gender", "Age")) %>% 
  mutate(diff = PLHIVold - PLHIVnew) %>% 
  .$diff %>% summary
```
