...
write_excel_csv(baseline.comp.data, "/gpfs/data/bershteynlab/EMOD/citrod01/MWI_calib_20220422/Analysis/mort_baseline_comp_data_20250515.csv")
```
...
p1 <- sim.results %>% 
  ...
  filter(Age >=15, Gender == 0, ceiling(Year) != Year) %>% 
...
p2 <- sim.results %>% 
  dplyr::select(Year, Gender, Age, Infected, sim.id, pop.scaling.factor) %>%
  filter(Age >=15, Gender == 1, ceiling(Year) != Year) %>% 
  group_by(Year, Age, Gender, sim.id) %>% 
  summarize(Infected = sum(Infected*pop.scaling.factor), .groups = 'keep') %>% 
  group_by(Year, Age) %>% 
...
for (i in (1:length(folder.list))){
  ...
  # On ART, less than 6 months after first initiation, CD4 < 200
  p <- mort.data %>% 
    filter(Total_number_of_times_initiating_ART == 1, Years_since_most_recent_ART_initiation <=.5) %>%
  ...
  # On ART, less than 6 months after first initiation, CD4 > 200
  p <- mort.data %>% filter(Total_number_of_times_initiating_ART == 1, Years_since_most_recent_ART_initiation <=.5) %>%
  ...
  # After interruption, on ART < 6 months after last re-initation, CD4 < 200
  p <- mort.data %>% filter(Total_number_of_times_initiating_ART > 1, Years_since_most_recent_ART_initiation <=.5) %>%
    filter(CD4_binned_InitART == 0) %>% 
    group_by(Death_year, Age_bin, Gender) %>% 
    count() 
  ...
  p <- mort.data %>% filter(Currently_in_ART == 1, CD4_count_current_binned == 1 ) %>% 
    group_by(Death_year, Age_bin, Gender) %>% 
    count()
...
## Total HIV+ population at mid year
```{r}
p1 <- sim.results %>% 
  dplyr::select(Year, Gender, Age, Infected, sim.id, pop.scaling.factor) %>%
  filter(Age >=15, Gender == 0, ceiling(Year) != Year) %>% 
...
## Total HIV deaths each year from Mortality Reports
...
mort.data.means <- merge(dat.plhiv %>% mutate(Year = Year- .5), mort.hiv.deaths , by = c("Year","Age")) %>% 
...
write_excel_csv(mort.data.means, '/gpfs/data/bershteynlab/EMOD/citrod01/MWI_calib_20220422/Analysis/mihpsa_mortality_table_20250515.csv')
...
merge(
  mort.report %>%
  group_by(Death_year) %>% summarize(Died = n()),
  mort.report %>% filter(Death_was_HIV_cause == 1) %>%
  group_by(Death_year) %>% summarize(Died_from_HIV = n()),
  by = c("Death_year")
)
