...
sim.results <- EMODAnalyzeR::read.simulation.results.bigpurple(
  ...
  experiment_path = "/gpfs/scratch/citrod01/experiments/Malawi/Baseline-campaign_Malawi_1Dec2021_recalib-Baseline___2022_04_13_14_14_43_389706",
...
This is leftover from the Zimbabwe calculations, so will need to adapt
...
circumcision.summary %>% 
  ggplot() + 
  geom_line(mapping = aes(x = Year, y = mean.fraction.circumcised))
```


```{r}
vmmc.count = sim.results %>% 
  dplyr::select(Year, Gender, Age, IsCircumcised, Population, sim.ix, pop.scaling.factor) %>%
  filter(Gender == 0, Age >= 15, Age < 50, ceiling(Year) != Year, IsCircumcised == 1) %>% 
...
vmmc.count = sim.results %>% 
  dplyr::select(Year, Gender, Age, IsCircumcised, Population, sim.ix, pop.scaling.factor) %>%
  filter(Gender == 0, Age < 50, ceiling(Year) != Year, IsCircumcised == 1, Year < 2010) %>% 
  group_by(Year, sim.ix) %>% 
  summarize(Population = sum(Population* pop.scaling.factor), .groups = 'keep') %>% 
  group_by(Year) %>% 
  summarize(Population = mean(Population))
