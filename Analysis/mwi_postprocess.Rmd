...
input_path = "/gpfs/scratch/citrod01/experiments/Malawi/Baseline-campaign_Malawi_20220422_recalib-MWI_Base___2024_09_16_10_42_24_238125"
...
sim.results.old <- EMODAnalyzeR::read.simulation.results.bigpurple(
  experiment_path = input_path,
  scenario_name = 'new',
  summarize_columns = c("Newly.Infected", "Newly.Tested.Positive", "Newly.Tested.Negative",
                        "Population","Infected", "On_ART","Died", "Died_from_HIV",
...
vmmc.summary <- sim.results %>% 
  mutate(Year = floor(Year)) %>% 
  dplyr::select(scenario_name, Year, Gender, Age, Program_VMMC, sim.ix, pop.scaling.factor) %>%
  filter(Gender == 0) %>% 
...
vmmc.summary %>% select(-scenario_name) %>% 
  pivot_longer(-Year) %>% 
  pivot_wider(names_from = Year, values_from = value)
```

Check: are there enough men alive at the time to receive vmmc?

```{r}
sim.results %>% 
  filter(Gender == 0, Age >=5, Age < 40) %>% 
  group_by(Year, sim.ix) %>% 
...
inc.plot <- ggplot() + 
  ...
  geom_line(data = inc.table %>% 
            ...
            mapping = aes(x = Year, y = mean.inc.overall, color = scenario_name), 
            size = 1) + #, color = "blue") +
...
vmmc.summary %>% 
  ggplot() + 
  geom_line(mapping = aes(x = Year, y = N_NewVMMC, color = scenario_name)) + 
  theme_bw()
```
